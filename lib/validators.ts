...
const sessionBaseSchema = z.object({
  ...
  subject: z.string().min(1).max(255).trim().optional(),
  ...
  comprehension: z.number().int().min(0).max(100).optional().nullable(),
...
export const createSessionSchema = sessionBaseSchema.refine((data) => !!data.task_id || !!data.subject, {
  message: 'Either task_id or subject is required',
  path: ['task_id'],
})
